    ...
    <>
      <header className="w-full relative flex justify-between bg-[#1b493d] px-5 lg:px-12 py-4" onKeyUp={closeProfileMenuByKey}>
        ...
        <div className="flex items-center">
          ...
          <ProfileNavigation profileMenuOverlay={profileMenu ? "block absolute left-0 top-0 w-full h-screen z-10" : undefined}  profileMenu={`absolute top-16 right-10 lg:right-20 bg-white rounded w-52 z-20  md:w-80 shadow-xl ${profileMenu ? "block" : "hidden"}`} closeProfileMenu={onShowProfileMenu}/>
        </div>
      </header>
      <MobileNavigation mobileMenuOverlay={mobileMenu ? "lg:hidden fixed left-0 top-0 w-full h-screen bg-black/70 z-20" : undefined}
